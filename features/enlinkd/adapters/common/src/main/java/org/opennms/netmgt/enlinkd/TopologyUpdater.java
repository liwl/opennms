...
public abstract class TopologyUpdater extends Discovery implements OnmsTopologyUpdater {
    ...
    public void unregister() {
        ...
        try {
            ...
            LOG.error("unregister: {}", e.getMessage());
        }
    }
    ...
    private <T extends OnmsTopologyRef>void delete(T topoObject) {
        try {
            ...
            LOG.error("Exception while deleting: {}", e.getMessage());
        }
    }
        ...
        if (!m_runned) {
            synchronized (m_topology) {
                try {
                    ...
                    LOG.error("run: {} first run: cannot build topology {}",getName(), e.getMessage());
                    return;
                }
            }
            ...
            try {
                ...
                LOG.error("cannot build topology {}", e.getMessage());
                return;
            }
            synchronized (m_topology) {
